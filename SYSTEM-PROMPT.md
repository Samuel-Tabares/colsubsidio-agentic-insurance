...
|---|---|---|
| `CAPA-CUALITATIVA.md` | El ICP, el dolor real, el futuro soñado, las 5 preguntas base, y el contenido original de las 6 objeciones | Base completa del prompt |
| Alex Hormozi, framework 3A Reframing (`hormozi-sales-blueprint.md`) | La estructura de tres pasos para manejar objeciones (Acknowledge, Associate, Ask), que `CAPA-CUALITATIVA.md` hacía sin el paso intermedio explícito | Sección de objeciones, las 6 reestructuradas |
...
| `UX.md` sección 10 | El ritmo de escritura: mensajes cortos, una pregunta por turno, sin urgencia falsa | "Cómo hablas" |
| `DEMO-HAPPY-PATH.md` | La regla de pivote de la pregunta 1, descubierta al construir el camino feliz para el perfil de 20 a 35 sin dependientes | "Las preguntas de discovery" |
| `BRIEF.md` (líneas 106, 185-207, nota del equipo del 2026-07-23) | Colsubsidio es sponsor, no asegurador ni intermediario; instrucción explícita para la Fase 3 que no se había ejecutado hasta la ronda 2 del dojo | Párrafo nuevo al inicio de la sección 1 |
...
Violan `CAPA-CUALITATIVA.md` sección 5 ("nunca insistir después de un no") y el criterio de
...
`CAPA-CUALITATIVA.md` ya prohibía antes de leer a Cialdini.

---

## 3. Contrato de herramientas (asumido, Fase 0 sin congelar)

Los contratos reales de `EMULADOR_ARQUITECTURA.md` no están cerrados todavía. Esto es lo que el
...
- ❌ "Este plan es de Colsubsidio, ellos te aseguran." (el campo `aseguradora` decía "Colsubsidio" como placeholder, no como aseguradora real; el brief prohíbe esta atribución explícitamente, `BRIEF.md:106,205-207`)
...
| 2026-07-25 | Al narrar el resultado de `recomendar()` (exclusiones vacías, con nota de que el catálogo aún no las tiene cargadas), el agente omitió el tema por completo — no inventó nada, pero tampoco declaró el vacío como pide la sección 4 ("exclusiones antes de que las pidan") y `DEMO-HAPPY-PATH.md` ("se declara así en pantalla, no se inventan") | Agregar a la sección 1 la instrucción explícita: si `recomendar()` devuelve exclusiones vacías, decir que ese dato falta y que un asesor lo confirma — nunca simplemente omitir el tema |
