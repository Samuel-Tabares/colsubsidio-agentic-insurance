...
export async function getOrCreateConversation(
  ...
  const inserted = await db
    ...
    .values({ id: newId("conversation"), organizationId, contactId, channel })
    .onConflictDoNothing()
    .returning();
...
export async function ingestInboundMessage(input: {
  ...
  /** Id externo del mensaje (wa_message_id en WhatsApp); si falta, se genera. */
  waMessageId?: string;
  type: string;
  text: string | null;
  timestamp: string;
  ...
  const externalId = input.waMessageId ?? `${channel}.in.${newId("message")}`;

  // Idempotencia dura: mismo id externo → sin efectos adicionales.
  const inserted = await db
