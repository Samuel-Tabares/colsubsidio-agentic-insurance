...
export const contact = pgTable(
  "contact",
  {
    ...
    notes: text("notes"),
    /**
     * Perfil compartido (Colsubsidio). `phone` se usa como id opaco del cliente
     * (celular en WhatsApp, id generado en web) — el handoff por id es la misma
     * fila. Estos campos los llena el cerebro por su lado.
     */
    perfilCrudo: jsonb("perfil_crudo"),
    analisis: jsonb("analisis"),
    analisisResumen: text("analisis_resumen"),
...
export const lead = pgTable(
  "lead",
  {
    ...
    /** UX §0.3: el sistema resolvió >70% pero el cierre necesita a un humano. */
    followManual: boolean("follow_manual").notNull().default(false),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
  },
  (t) => [
...
export const conversation = pgTable(
  "conversation",
  {
    ...
    contactId: text("contact_id")
      ...
      .references(() => contact.id, { onDelete: "cascade" }),
    /**
     * Canal de la conversación. `whatsapp` conserva el flujo Meta (ventana 24h,
     * envío por Graph); `web` responde siempre y entrega por SSE, sin Meta.
     */
    channel: text("channel", { enum: ["whatsapp", "web"] })
      .notNull()
      .default("whatsapp"),
...
export const message = pgTable(
  "message",
  {
    ...
    /**
     * text | interactive | recomendacion | comparacion | control_cobertura | cierre.
     * Se deja como texto libre (no enum) para no migrar por cada tarjeta nueva.
     */
    type: text("type").notNull().default("text"),
    text: text("text"),
    /** Contenido estructurado de tarjetas interactivas (lo produce el cerebro). */
