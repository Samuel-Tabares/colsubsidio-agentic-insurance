...
- `scripts/gen_dashboard_stats.py` and `scripts/cross_analysis.py` (both need `pandas`/`numpy` — install in a venv, e.g. `python3 -m venv .venv && .venv/bin/pip install pandas numpy`) regenerate `output/dashboard_stats.json` and `output/cross_stats.json` respectively from `output/afiliados_clean.csv`. Both were rewritten 2026-07-23 for the new 13-field schema (was 12) and re-run against the new 500k dataset — the exploratory dashboard artifact linked in `README.md` was rebuilt from their output the same day. Re-run both after any change to the ETL output.
  - `cross_analysis.py` no longer hardcodes which fields have missingness (the old dataset's 4-field socioeconomic cluster doesn't exist here) — it computes `FIELDS_WITH_MISSING` from the data each run and only masks pairs involving those fields.
  - **Key finding**: `rango_salarial|categoria` is the strongest pair in the whole dataset (V=0.90, near-deterministic) — `categoria`'s four opaque tokens function as a 3-tier income bucket + "sin dato" (SIGMA≈<2 SMLV, PI≈2–4 SMLV, ZETA≈>4 SMLV, MU≈no salary data), derivable from the real `rango_salarial` field without needing the missing codebook. The other three opaque fields (`segmento_grupo_familiar`, `segmento_poblacional`, `piramide_empresa`) don't have as clean an external anchor — they relate to each other and to age/gender (V 0.15–0.48) but can't currently be named in plain language. `piramide_empresa|empresa_id` (V=0.84) repeats the old `piramide_empresa|empresa_asociada` structural pattern: one `empresa_id` value is a near-exclusive flag for one `piramide_empresa` token.
...
- **Unconfirmed hypothesis: the opaque tokens in `segmento_grupo_familiar`, `segmento_poblacional`, and `piramide_empresa` may be frequency-rank-decodable against the old dataset's real category labels**, the same way `categoria` was decoded via `rango_salarial` — but this is weaker evidence (cross-sample frequency correlation, not a same-sample statistical dependency) and the team's plan is to ask Colsubsidio directly for the real codebook rather than rely on this. Documented here so the reasoning isn't lost:
  ...
  - None of this should be used to label things in the product without Colsubsidio confirming the real codebook — it's directional, not proof.
